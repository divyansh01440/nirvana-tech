...
// ✅ Fix: use explicit ComponentType instead of React.ElementType
...
export default function Sidebar(props: SidebarProps) {
  // ✅ Fix: destructure from props, not directly in params
  const { userName, userRole, queryCount = 0 } = props;
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 h-16 border-b border-white/5 flex-shrink-0">
        ...
        <button
          ...
          className="w-6 h-6 rounded-md flex items-center justify-center text-white/40 hover:text-nirvana-white hover:bg-white/5 transition-all flex-shrink-0"
        ...
        {!collapsed ? (
          <div className="space-y-2">
            ...
            <button
              ...
              disabled={signingOut}
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-white/40 hover:text-red-400 hover:bg-red-500/10 transition-all text-xs disabled:opacity-50"
            >
          ...
          <div className="space-y-2 flex flex-col items-center">
            ...
            <button
              ...
              disabled={signingOut}
              title="Sign out"
              className="w-8 h-8 rounded-xl flex items-center justify-center text-white/40 hover:text-red-400 hover:bg-red-500/10 transition-all disabled:opacity-50"
            >
              <LogOut size={14} />
            </button>
          </div>
...
// ✅ Fix: use props pattern, not destructured params
function NavLink(props: NavLinkProps) {
  const { item, pathname, collapsed } = props;
    ...
    <Link
      ...
      className={[
        ...
        isActive
          ...
          : "text-white/50 hover:text-nirvana-white hover:bg-white/5 border-l-2 border-transparent",
      ].join(" ")}
    >
      {/* Active dot */}
      {isActive && (
      ...
      {/* Icon — ✅ size is now valid because IconType accepts size?: number */}
      <Icon
        size={16}
